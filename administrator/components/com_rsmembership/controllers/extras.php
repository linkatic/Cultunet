...
class RSMembershipControllerExtras extends RSMembershipController
{
	...
	function saveOrder()
	{
		...
		// Redirect
		$this->setRedirect('index.php?option=com_rsmembership&view=extras', JText::_('RSM_EXTRAS_ORDERED'));
	}
	...
	function changestatus()
	{
		...
		else
		{
			...
			$total = count($cid);
			if ($value)
				$msg = JText::sprintf('RSM_EXTRAS_PUBLISHED', $total);
			else
				$msg = JText::sprintf('RSM_EXTRAS_UNPUBLISHED', $total);
		...
		// Redirect
		$this->setRedirect('index.php?option=com_rsmembership&view=extras', $msg);
	}
	...
	function remove()
	{
		...
		else
		{
			...
			$total = count($cid);
			$msg = JText::sprintf('RSM_EXTRAS_DELETED', $total);
	...
	function save()
	{
		...
		{
			case 'apply':
				$link = 'index.php?option=com_rsmembership&controller=extras&task=edit&cid='.$cid;
				if ($result)
					$this->setRedirect($link, JText::_('RSM_EXTRA_SAVED_OK'));
				else
					$this->setRedirect($link, JText::_('RSM_EXTRA_SAVED_ERROR'));
			break;
		
			case 'save':
				$link = 'index.php?option=com_rsmembership&view=extras';
				if ($result)
					$this->setRedirect($link, JText::_('RSM_EXTRA_SAVED_OK'));
				else
					$this->setRedirect($link, JText::_('RSM_EXTRA_SAVED_ERROR'));
