...
function submitbutton()
{
	...
	rsmembership_submit_form_ajax('addmembershipmodules');
	<?php } else { ?>
	rsmembership_submit_form_ajax('addextravaluemodules');
	<?php } ?>
...
<?php if (!$this->has_patches) { ?>
	<p><strong><?php echo JText::_('RSM_PATCHES_NOT_APPLIED'); ?></strong></p>
<?php } ?>

<form action="<?php echo JRoute::_('index.php?option=com_rsmembership&view=share&layout=module&tmpl=component&'.$this->what.'='.$this->id); ?>" method="post" name="adminForm">
	...
	<div id="editcell1">
		<table class="adminlist">
			...
			<tr>
				...
				<th width="20"><input type="checkbox" name="toggle" value="" onclick="checkAll(<?php echo count($this->modules); ?>);"/></th>
				<th><?php echo JHTML::_('grid.sort', 'Title', 'title', $this->sortOrder, $this->sortColumn); ?></th>
				<th><?php echo JHTML::_('grid.sort', 'Module', 'module', $this->sortOrder, $this->sortColumn); ?></th>
				<th width="1%"><?php echo JHTML::_('grid.sort', 'Published', 'published', $this->sortOrder, $this->sortColumn); ?></th>
				<th width="1%"><?php echo JHTML::_('grid.sort', 'Position', 'position', $this->sortOrder, $this->sortColumn); ?></th>
				<th width="1%"><?php echo JHTML::_('grid.sort', 'Client', 'client_id', $this->sortOrder, $this->sortColumn); ?></th>
	...
	?>
		<tr class="row<?php echo $k; ?>">
			...
			<td align="center"><?php echo JHTML::image($row->published ? 'administrator/images/tick.png' : 'administrator/images/publish_x.png', ''); ?></td>
			<td align="center" nowrap="nowrap"><?php echo $row->position; ?></td>
	...
	<input type="hidden" name="layout" value="module" />
