...
$list .= "first_name, last_name, user_email FROM #__{vm}_orders, #__{vm}_order_user_info WHERE ";
...
while ($db->next_record()) { 
	...
	if ($db->f('first_name')!='' || $db->f('last_name')!='')	
	...
	if( $perm->check('admin') && defined('_VM_IS_BACKEND')) {
		...
		if($tmp_cell == '')
			$tmp_cell = '<a href="'.$sess->url( $url ).'">'.$db->f('user_email').'</a>';
		else 
