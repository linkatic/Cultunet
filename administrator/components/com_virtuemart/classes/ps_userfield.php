...
class vm_ps_userfield extends vmAbstractObject {
	...
	function listUserFields( $rowFields, $skipFields=array(), $db = null, $startForm = true ) {
		...
		//Hack (cambio de consulta) en backend por la incorporación de dos nuevos grupos de usuario
	    //Si no se utiliza no carga los datos del formulario	    
	    if($page == "admin.user_form")
		{
		    $q = "SELECT * FROM #__users AS u LEFT JOIN #__vm_user_info AS ui ON id=user_id WHERE id=".JRequest::getVar('user_id')." AND (address_type='BT' OR address_type IS NULL ) AND gid <= 33"; 		 
		    $db->setQuery($q);
			$db->query($q);
		}
