...
class vm_ps_checkout {
	...
	function noShipToNecessary() {
		...
		//Buscamos productos no descargables
		if( ENABLE_DOWNLOADS == '1') {
			...
			for($i = 0; $i < $cart["idx"]; $i++) {
			 ...
			 $no_ship = !$not_downloadable;
		}
		
		if(!$no_ship)
		{
			//Buscamos productos no suscripciones digitales
			$not_digital_suscription = false;
			for($i = 0; $i < $cart["idx"]; $i++) {
				if( !ps_product::is_digital_suscription($cart[$i]['product_id']) && !ps_product::is_downloadable($cart[$i]['product_id'])) {					
					$not_digital_suscription = true;
					break;
				}
			}
			return !$not_digital_suscription;
		} else return $no_ship;
	...
	function list_payment_methods( $payment_method_id=0 ) {
		...
		//Mod para mostrar/ocultar el método de pago 'Domiciliación bancaria'
		//Este método solamente debe estar disponible en 
		//en el servicio de suscripción revista física (excepto internacional)
		...
		//print_r($_SESSION['cart']);
			...
			for ($i=0;$i<$_SESSION['cart']["idx"];$i++) {
				...
				if(($category_id!=2) || ($category_id==2 && $product_id==3)) $pago_domiciliado = 0;
			}
			
			//echo 'Domiciliado: '. $pago_domiciliado;
			
		//Mod para ocultar el método de pago 'Transferencia bancaria' en las suscripciones digitales
			$pago_transferencia = 1;
			for ($i=0;$i<$_SESSION['cart']["idx"];$i++) {
				$product_id = $_SESSION['cart'][$i]['product_id'];
				$category_id = $_SESSION['cart'][$i]['category_id'];
							
				if(($category_id==23) || ($category_id==3)) $pago_transferencia = 0;
			}
		...
		if(!$pago_domiciliado) $q .= "#__{vm}_payment_method.payment_method_id!='22' AND ";
		if(!$pago_transferencia) $q .= "#__{vm}_payment_method.payment_method_id!='20' AND ";
		...
		$theme->set_vars(array('db_nocc' => $db_nocc,
								...
								'pago_domiciliado' => $pago_domiciliado
						 	)
						 );
...
----------------------------' 
    ...
    */
	function email_receipt($order_id) {
		...
		$qt = "SELECT * FROM #__{vm}_user_info WHERE user_id='".$user_id."' AND address_type='BT' AND user_email!='NULL'";
		$dbbt->query($qt);
		$dbbt->next_record();
