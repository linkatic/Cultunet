...
{	
    ...
    function addGroup()
    {
    	...
    	require_once(JPATH_COMPONENT.DS.'models'.DS.'option.php');
		$option_model = new ModelOption();
		$colorpickers = (int) $option_model->getOption('colorpickers');
    	//
    	require_once(JPATH_COMPONENT.DS.'views'.DS.'navigation.view.php');
    ...
    function editGroup()
    {
		...
		$slides_model = new ModelSlide();
		$slide_list = $slides_model->getSlides($cid[0]);    	
    	//
    	if($data !== FALSE)
    	{
			require_once(JPATH_COMPONENT.DS.'models'.DS.'option.php');
    ...
    function viewGroup()
    {
    	...
    	//
    	require_once(JPATH_COMPONENT.DS.'models'.DS.'slide.php');
		$group_slide = new ModelSlide();
		$cid = JRequest::getVar( 'cid', array(0), '', 'array' );
		$rows = $group_slide->getSlides($cid[0]);    	
		require_once(JPATH_COMPONENT.DS.'models'.DS.'option.php');
		$option_model = new ModelOption();
		$slide_shortcuts = (int) $option_model->getOption('slide_shortcuts');
    	//
    	if($rows !== FALSE)
    	{
			//
...
/* Location: ./views/gk3_photoslide/group.view.php */