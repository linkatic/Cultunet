			...
			while( $this->fields->hasNext() ) {
				...
				if(!in_array($field->name,array('link_hits','link_votes','link_rating','link__featured'))) {
					if( ($field->isRequired() && !in_array($field->name,array('link_name','link_desc'))) || ($field->isRequired() && $this->mtconf['use_wysiwyg_editor'] == 0 && $field->name == 'link_desc') ) {
						...
						{
							echo "\n";
							echo '} else if (form.'. $field->getName().' && ';
						...
						{
							echo '} else if (form.'. $field->getName().' && isEmpty(\'' . $field->getName() . '\')) {'; 
							echo 'alert("' . addslashes(JText::_( 'Please complete this field' ) . $field->caption) . '");';
						}
					}
...
<table id="addlisting-form" width="100%" cellpadding="0" cellspacing="4" border="0" align="center">
	...
	<tr align="bottom">
		<td width="30%" align="left" valign="top"><?php echo JText::_( 'Category' ) ?>:</td>
	...
	while( $this->fields->hasNext() ) {
		...
		if($field->hasInputField()) {
			echo '<tr id="field'.$field->getId() .'"><td valign="top" align="left">';
...
/*	var fields = document.getElementById('addlisting-form').getElementsByTagName('tr');
	...
	//Actualizamos la vista del formulario en función de la categoría seleccionada
	updateFormAddRecurso();
