<?php
/**
 * @version		$Id: mtree.php 904 2010-06-28 08:54:37Z cy $
 * @package		Mosets Tree
...
function editlisting( $link_id, $option ) {
	...
	$sql = "SELECT cf.*, " . ($link_id ? $link_id : 0) . " AS link_id, cfv.value AS value, cfv.attachment, cfv.counter, ft.ft_class FROM #__mt_customfields AS cf "
		...
		.	"\nLEFT JOIN #__mt_fieldtypes AS ft ON ft.field_type=cf.field_type"
		.	"\nINNER JOIN #__mtcats_customfields AS cxf ON cxf.id_customfield=cf.cf_id"
		.	"\nWHERE cf.hidden ='0' AND cf.published='1' AND cxf.id_cat=".$cat_id." ORDER BY ordering ASC";
	$database->setQuery($sql);
	...
	if ( ($mtconf->get('user_addlisting') == 1 && $my->id < 1) || ($link_id > 0 && $my->id == 0) ) {
		...
		#Comprobamos si el grupo está relacionado con la categoria
		$database->setQuery( "SELECT * FROM #__mtcats_aclgroups WHERE id_cat = '$cat_id' AND id_group = '".$my->get('gid')."'");
		$result = $database->loadResultArray();
	
		#Si la consulta no devuelve nada el grupo no puede añadir recursos en la categoría
		if(count($result)) $savant->display( 'page_addListing.tpl.php' );
		else $savant->display( 'page_errorNoAddListing.tpl.php' );
	}
}
