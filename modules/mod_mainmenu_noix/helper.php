...
// Get permission on item
      ...
      $prequery1 = "select gid, usertype from #__users where id = " . $user->id;
      $db->setQuery( $prequery1 );
      $row = $db->loadObject();
      $arrGroupIds[] = $row->gid;
      $arrGroupNames[] = $row->usertype;
      
      $prequery2 = "select id_group, name from #__noixacl_multigroups, #__core_acl_aro_groups where id_user = " . $user->id;
      ...
      $query = "select * from  #__noixacl_rules where axo_value = " . $item->id . " and aro_value in (" . $inGroups;
      ...
      $query = "select access from  #__menu where id = " . $item->id;
      $db->setQuery($query);
      $row = $db->loadObject();
      $publicaccess = false;
      if ($row->access == 0) {
