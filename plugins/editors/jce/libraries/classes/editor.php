	...
	*/
	...
	{		
		...
		foreach ($plugins as $plugin) {			
			...
			// TinyMCE will fail to load if a language file for a plugin is missing
			if ($language != 'en') {
				// new language file
				...
				if (JFile::exists($en) && !JFile::exists($new)) {
					...
					if (!@JFile::copy($en, $new)) {
						...
						$this->removeKeys($plugins, $plugin);
						JError::raiseNotice('SOME_ERROR_CODE', sprintf(JText::_('PLUGIN_REMOVED_LANG_FILE_MISSING'), 'plugins/editors/jce/tiny_mce/plugins/'.$plugin.'langs/'.$language.'.js') . ' - ' . ucfirst($plugin));
					}
				}
			}
		}
