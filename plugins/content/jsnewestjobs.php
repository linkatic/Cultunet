 ...
 * Created on:	Dec 10, 2009
 ^
 + Project: 		JS Jobs 
 * File Name:	Pplugin/jsnewestjobs.php
 ^ 
...
class plgContentJSNewestJobs extends JPlugin
{
        public function onPrepareContent( &$row, &$params, $page=0 )
        {
                if ( JString::strpos( $row->text, 'jsnewestjobs' ) === false ) {
                        return true;
                }

              // expression to search for
                $regex = '/{jsnewestjobs\s*.*?}/i';
        ...
        protected function _process( &$row, &$matches, $count, $regex, $style )
        {
                for ( $i=0; $i < $count; $i++ )
                {
                        $load = str_replace( 'jsnewestjobs', '', $matches[0][$i] );
