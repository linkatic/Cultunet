...
class NoNumberLicenses
{
	...
	function blockHTML( $extension = '', $alias = '', $state = '', $host = '', $addmargin = 0 )
	{
		...
		switch ( $state ) {
			case 'valid':
				$text = JText::sprintf( '-This is a commercial version', $extension, $host );
				$bgcolor = '#F6F6F6';
				$color = '#009900';
				$formcolor = '#999999';
				$bordercolor = '#EEEEEE';
				break;
			case 'invalid':
				$text = JText::sprintf( '-The License code is not valid', $extension, $host );
				break;
			case 'local':
				$text = JText::sprintf( '-Cannot check if License code is valid because you are working on a local server',  $extension );
				break;
			case 'fail':
				$text = JText::sprintf( '-Cannot check if License code is valid', $extension );
				break;
			default:
		...
		$html[] = '<div style="border:1px solid #CCCCCC;margin-bottom:'.$margin.'"><div style="padding: 2px 5px;background-color:'.$bgcolor.';">';
			...
			if ( $state != 'valid' ) {
				...
				$html[] = '<span style="white-space:nowrap;"><em>';
					$html[] = '(<a href="http://www.nonumber.nl/'.$alias.'/license" target="_blank">'.JText::_( '-Purchase License code' ).'</a>';
					$html[] = ( $host ? ' '.JText::sprintf( '-for your domain', $host ) : '' ).')';
				$html[] = '</em></span>';
			}
		$html[] = '</div></div>';
